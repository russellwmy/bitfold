...
mod tests {
    ...
    fn test_dropped_packets_wraparound() {
        ...
        for _ in 0..65530 {
            handler.process_outgoing(PacketType::Packet, b"x", OrderingGuarantee::None, None, now);
        }
        ...
        assert_eq!(
            dropped.len(),
            0,
            "Packets within ACK window should not be dropped during wraparound"
        );
    }
    ...
    fn test_dropped_packets_window_edge() {
        ...
        handler.process_outgoing(PacketType::Packet, b"test", OrderingGuarantee::None, None, now);
