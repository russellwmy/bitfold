    ...
    fn process_event(
        ...
        match event.delivery_guarantee() {
            ...
            DeliveryGuarantee::Unreliable => {
                use crate::protocol::packet::OrderingGuarantee;
                ...
                match ordering {
                    OrderingGuarantee::Unsequenced => {
                        ...
                        let compression_overhead = match self.config().compression {
                            crate::core::config::CompressionAlgorithm::Lz4 => 5,
                            _ => 1,
                        };
                        ...
                        let base = crate::core::shared::SharedBytes::from_arc(event.payload_arc());
                        ...
                        while offset < base.len() {
                            ...
                            self.enqueue_command(
                                crate::protocol::command::ProtocolCommand::SendUnsequenced {
                                    channel_id,
                                    unsequenced_group,
                                    data: chunk,
                                },
...
mod tests {
    ...
    use crate::{
        host::Session,
        protocol::{command::ProtocolCommand, command_codec::CommandDecoder},
    };
    ...
    fn incoming_bandwidth_limit_drops_excess_packets() {
        ...
        let start = Instant::now();
        let client_cfg = crate::core::config::Config::default();
        ...
        let mut server_cfg = crate::core::config::Config::default();
