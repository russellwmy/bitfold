...
use crate::{
    core::{
        config::Config, error::Result, interceptor::Interceptor,
        transport::Socket as TransportSocket,
    },
    peer::Peer,
    protocol::packet::{DeliveryGuarantee, OrderingGuarantee, Packet},
};
...
impl Host {
    ...
    /// ```no_run
    /// use crate::host::Host;
    /// use crate::core::{config::Config, interceptor::Interceptor};
...
mod tests {
    ...
    fn test_broadcast_with_different_delivery_guarantees() {
        ...
        let count2 = host
            .broadcast(1, vec![4, 5, 6], DeliveryGuarantee::Unreliable, OrderingGuarantee::None)
            .unwrap();
        assert_eq!(count2, 0);
    }
    ...
    impl CountingInterceptor {
        fn new() -> Self {
            Self { received: Arc::new(Mutex::new(0)), sent: Arc::new(Mutex::new(0)) }
        }
    ...
    fn test_interceptor_counts_packets() {
        ...
        assert!(counter_clone.received_count() > 0, "Interceptor should count received packets");
    }
    ...
    fn test_interceptor_can_modify_packets() {
        ...
        let mut server =
            Host::bind_with_interceptor("127.0.0.1:0", config.clone(), Box::new(XorInterceptor))
                .unwrap();
        let server_addr = server.local_addr().unwrap();
