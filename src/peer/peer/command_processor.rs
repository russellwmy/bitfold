...
use crate::{
    ...
    peer::{
        channel_state::ChannelState, fragment_buffer::CommandFragmentBuffer, peer_state::PeerState,
            ...
            ProtocolCommand::SendReliable { channel_id, sequence, ordered, data } => {
                ...
                if *ordered {
                    ...
                    let ordered_packets = channel_state.process_ordered(
                        *sequence,
                        crate::core::shared::SharedBytes::from_arc(
                        ...
                        if let Some(reassembled) = buffer.reassemble() {
                            ...
                            if is_ordered {
                                ...
                                let ready_packets = channel_state.process_ordered(
                                    *sequence,
                                    crate::core::shared::SharedBytes::from_vec(reassembled),
                                );
...
mod tests {
    use super::*;
    use crate::{core::config::Config, protocol::command::ProtocolCommand};
