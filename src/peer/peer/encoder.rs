...
impl Peer {
    ...
    pub fn encode_queued_commands_bounded(
        ...
        for cmd in self.command_queue.iter() {
            let encoded = crate::protocol::command_codec::CommandEncoder::encode_command(cmd)?;
        ...
        let mut scratch = self.tx_pool.allocate();
        scratch.clear();
        crate::protocol::command_codec::CommandEncoder::encode_packet_into(&mut scratch, &packet)?;
...
mod tests {
    ...
    fn test_mtu_boundary_calculation() {
        ...
        config.compression = crate::core::config::CompressionAlgorithm::None;
        let mut peer = Peer::new(get_fake_addr(), &config, Instant::now());
