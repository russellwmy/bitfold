...
use super::Peer;
use crate::{core::shared::SharedBytes, protocol::command::ProtocolCommand};
...
impl Peer {
    ...
    pub fn enqueue_reliable_data(&mut self, channel_id: u8, data: Arc<[u8]>, ordered: bool) -> u16 {
        ...
        let compression_overhead = match self.config.compression {
            crate::core::config::CompressionAlgorithm::Lz4 => 5, // marker + original size
            _ => 1,                                              // marker only
        };
...
mod tests {
    ...
    use crate::{
        core::config::Config,
        protocol::{command::ProtocolCommand, packet::DeliveryGuarantee},
    };
    ...
    fn test_multiple_stale_fragments_cleanup() {
        ...
        assert_eq!(
            peer.command_fragments.len(),
            0,
            "All stale fragment buffers should be cleaned up"
        );
    }
}
