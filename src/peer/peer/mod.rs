...
use super::{
    ...
    pmtu_discovery::PmtuDiscovery,
    statistics::PeerStatistics,
    unsequenced::UnsequencedState,
};
use crate::{
    core::{config::Config, packet_pool::PacketAllocator},
    protocol::{command::ProtocolCommand, AcknowledgmentHandler, SentPacket},
};
...
pub struct Peer {
    ...
    compression_pool: crate::core::packet_pool::CompressionBufferPool,
...
impl Peer {
    ...
    pub fn new(addr: SocketAddr, config: &Config, time: Instant) -> Peer {
        ...
        Peer {
            ...
            compression_pool: crate::core::packet_pool::CompressionBufferPool::default(),
...
mod tests {
    ...
    fn test_pmtu_discovery_probe_reply() {
        ...
        if let Some((size, token, _)) = peer.pmtu.outstanding_probe() {
            let reply = crate::protocol::command::ProtocolCommand::PMTUReply { size, token };
    ...
    fn test_pmtu_discovery_enabled_by_default() {
        ...
        let probe_time =
            creation_time + std::time::Duration::from_millis(config.pmtu_interval_ms as u64 + 100);
